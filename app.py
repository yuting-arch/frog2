...
# --- 1. 頁面設定 ---
st.set_page_config(layout="wide", page_title="台灣蛙鳴聲景：漣漪沉浸地圖")
...
st.markdown("""
    ...
    <p style='text-align: center; color: #666; font-size: 1em;'>
        每一聲紀錄在夜色中泛起 #C4E1FF 的柔和漣漪，靜謐地訴說著台灣的生命力。
    </p>
...
try:
    ...
    # --- 3. 沉浸式漣漪地圖 HTML/JS ---
    def create_ripple_map(df_r, df_v):
        ...
        <style>
            ...
            /* 核心 6px 定點 (移除掉落動畫，改為原位呼吸浮現) */
            .core-point {{
                position: absolute; width: 6px; height: 6px; border-radius: 50%;
                transform: translate(-50%, -50%); opacity: 0;
                animation: fade-in-core 1.2s ease-out forwards;
            }}
            ...
            .ripple {{
                ...
                animation: ripple-out 4.5s ease-out forwards;
            }}
        ...
        <script>
            ...
            function playData(data, colorClass, baseDelay) {{
                data.forEach((row, i) => {{
                    setTimeout(() => {{
                        ...
                        dot.className = 'core-point ' + colorClass;
                        ...
                        const createRipple = (delay) => {{
                            ...
                            rip.style.animationDelay = delay + 's';
                            layer.appendChild(rip);
                        }};

                        createRipple(0);
                        createRipple(1.5); // 第二道漣漪延遲，增加水面擴散感
            ...
            playData(verData, 'ver-color', 2500); 
        </script>
        """
        return html_content

    if not raw_data.empty or not verified_data.empty:
        st.components.v1.html(create_ripple_map(raw_data, verified_data), height=660)
    else:
